// Updated Component with Fully Flexible Draggable Divider - No Height Reset
...
// --- MOCK DATA & CONFIGURATION --- (same as before)
const View = {
  DAY: 'Day',
  WEEK: 'Week',
  MONTH: 'Month',
...
export default function DataEntriesContent() {
  ...
  // Split view state - now always on, but with toggle to show/hide bottom panel
  ...
  const toggleBottomPanel = () => {
    ...
    // No height reset - let it remember the last dragged position for flexibility
  };

  // Draggable divider logic - fully flexible up and down, no constraints beyond min/max
  useEffect(() => {
    const divider = dividerRef.current;
    if (!divider || !showBottomPanel) return;
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      startHeight = containerHeight * (topPanelHeight / 100);
  ...
  }, [showBottomPanel, topPanelHeight, isDragging]);
  ...
  return (
    <div className="h-screen overflow-hidden font-sans antialiased">
      ...
      {/* Header: Fixed at top */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-white border-b shadow-sm z-20">
      ...
      <div className="flex-1 flex flex-col" ref={containerRef}>
        {/* Top Panel: Main Content - Flexible height */}
        <div 
          className="top-panel bg-white relative" 
        ...
        >
          ...
          <div className="p-4">
            ...
            {/* Data Table - full remaining space in top panel */}
        ...
        {/* Draggable Divider - only show if bottom panel is visible */}
        {showBottomPanel && (
          <div 
            ref={dividerRef}
        ...
        {/* Bottom Panel: Files Preview - Flexible height */}
        {showBottomPanel && (
          <div 
            className="bottom-panel bg-gray-50 p-4 grid grid-cols-2 gap-4"
